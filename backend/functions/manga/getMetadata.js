...
export default async function getMangaMeta({ urlName, host }) {
	...
	const html = await fetch(detailsPage.url.replace('%name', urlName), { redirect: 'follow' }).then(res => res.text());
	const document = HTMLParser.parse(html);

	const entries = ['title', 'description', 'otherNames', 'authors', 'artists', 'genres', 'released', 'airStatus'];
	const mangaMeta = {};
	for (const entry of entries) {
		if (!detailsPage[entry]) continue;
		mangaMeta[entry] = document.querySelector(detailsPage[entry])?.textContent?.trim();
	}
