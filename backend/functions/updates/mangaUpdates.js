...
import asyncMap, { asyncSettled } from '../asyncMap.js';
...
export default async function mangaUpdates() {
	...
	await asyncMap(ongoingMangas, async manga => {
		...
		let mangaMeta;
		try {
			mangaMeta = await getMangaMeta({ urlName: manga.sourceUrlName, host });
		} catch (err) {
			console.log('---------------');
		...
		const newChapters = mangaMeta.chapters.filter(chapter =>
		...
		await asyncSettled(newChapters, async chapter => {
			await sendDiscordWebhook({
				...
				embeds: [{
					...
					url: new URL(`/mangas/${manga.urlName}/${chapter.urlName}`, process.env.WEBSITE_URI).href,
				}],
			});
		});
	});

	console.log(chalk.blue(`${updatedMangaCount} updated mangas, ${newChapterCount} new chapters`));
}
