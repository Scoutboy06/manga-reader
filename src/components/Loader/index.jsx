...
export default function Loader({ size, style }) {
	return (
		...
		<div className={styles.lds_spinner} style={{...style, transform: `scale(${size / 80})` }}><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>
	)
}