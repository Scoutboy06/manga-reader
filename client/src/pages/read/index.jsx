...
export default function Read() {
	...
	const paginate = dir => {
		...
		if (dir === -1) navigate(`/read/${mangaMeta.urlName}/${chapters.prev}`);
		if (dir === 1) navigate(`/read/${mangaMeta.urlName}/${chapters.next}`);
	};
	...
	const scrollHandler = () => {
		...
		for (let i = 0; i < images.length; i++) {
			const img = imageSizes.current[i];

			if (window.scrollY + window.innerHeight * 0.5 > img.y) {
				currentImageIndex = i;
			}
		}
	...
	const setImagesTopCoords = (start = 0, end = images.length) => {
		for (let i = start; i < end; i++) {
			...
			imageSizes.current[i] = { y: y + window.scrollY, height };
		}
